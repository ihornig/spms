...
public class TerminHandler {
	...
	public String speichern(ActionEvent event) {
		if(abfrage()) {
			String name = (String) event.getComponent().getAttributes().get("username");
			tempTermin.setAutor(name);
			try {
			...
			return "Es existiert bereits ein Termin mit diesem Titel.";
		}
	}
	...
	public boolean abfrage() {
		...
		while(!found && i < termine.getRowCount()) {
			...
			if(tempTermin.getTitel().equals(anotherTempTermin.getTitel())) {
				found = true;
			}
		}
		if(!found) {
